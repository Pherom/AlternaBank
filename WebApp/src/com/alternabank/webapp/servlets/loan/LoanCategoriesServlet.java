...
@WebServlet(name = "LoanCategoriesServlet", urlPatterns = "/loan-categories")
public class LoanCategoriesServlet extends HttpServlet {
        ...
        if (isAuthorized) {
            ...
            synchronized (getServletContext()) {
                if (!timeManager.isRewindMode())
                    categories = loanManager.getAvailableCategories();
                else categories = loanManager.getAvailableCategories(timeManager.getCurrentTime());
            }
            String loanDetailsJSON = WebAppUtils.GSON_INSTANCE.toJson(categories);
